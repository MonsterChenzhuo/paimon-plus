...
pub type GetObjectByteStream = Pin<Box<dyn Stream<Item = Result<Bytes>> + Send>>;
...
impl GetObjectFluentBuilder {
    ...
    /// Send the request and buffer the full object in memory.
    pub async fn send(&self) -> Result<GetObjectOutput> {
        let resp = self.send_request().await?;
    ...
    pub async fn send_streaming(&self) -> Result<GetObjectStreamingOutput> {
        let resp = self.send_request().await?;
        ...
        let body = resp
            .bytes_stream()
            .map(|chunk| chunk.map_err(ObsError::from));

        Ok(GetObjectStreamingOutput {
            body: Box::pin(body),
            content_type,
            content_length,
            etag,
...
/// Output for the GetObject streaming operation.
pub struct GetObjectStreamingOutput {
    body: GetObjectByteStream,
    content_type: Option<String>,
    content_length: Option<u64>,
    etag: Option<String>,
    last_modified: Option<String>,
...
impl std::fmt::Debug for GetObjectStreamingOutput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GetObjectStreamingOutput")
            .field("body", &"Stream(..)")
...
impl GetObjectStreamingOutput {
    /// Consume the output and get the body stream.
    pub fn into_body(self) -> GetObjectByteStream {
        self.body
    }
