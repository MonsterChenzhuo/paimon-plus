...
impl Authorization for Client {
    ...
    fn auth(
        ...
        if let Some(security_token) = self.config().credentials().security_token() {
            headers.insert(
                "x-obs-security-token".into(),
                vec![security_token.to_string()],
            );
        }
...
mod tests {
    ...
    #[test]
    fn test_temporary_credentials_add_security_token_to_signed_headers() {
        use crate::config::{Config, Credentials};

        let config = Config::builder()
            .credentials(Credentials::new_with_token("ak", "sk", "token-1"))
