  ...
  override def newScanBuilder(options: CaseInsensitiveStringMap): ScanBuilder = {
    table match {
      case t: KnownSplitsTable =>
        new PaimonSplitScanBuilder(
          t.copy(OptionUtils.withNativeIOReadOptions(options.asCaseSensitiveMap))
            .asInstanceOf[KnownSplitsTable])
      case _: InnerTable =>
        new PaimonScanBuilder(
          table
            .copy(OptionUtils.withNativeIOReadOptions(options.asCaseSensitiveMap))
