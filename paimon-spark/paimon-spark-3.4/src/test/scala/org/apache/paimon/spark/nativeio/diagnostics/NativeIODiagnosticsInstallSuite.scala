...
class NativeIODiagnosticsInstallSuite extends FunSuite {

  test("installs Native IO tab from session extension when native io is enabled") {
    val spark =
      SparkSession
        .builder()
        .master("local[1]")
        .appName("native-io-diagnostics-install")
        .config("spark.ui.enabled", "true")
        .config("spark.ui.port", "0")
        .config("spark.sql.extensions", classOf[PaimonSparkSessionExtensions].getName)
        .config("spark.paimon.native-io.enabled", "true")
        .getOrCreate()
