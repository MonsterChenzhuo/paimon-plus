...
public class ReadBuilderImpl implements ReadBuilder {
    ...
    private final int nativeReadOptionsFingerprint;
    ...
    public ReadBuilderImpl(InnerTable table) {
        ...
        this.nativeReadOptionsFingerprint = nativeReadOptionsFingerprint(table.options());
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(table.name(), that.table.name())
                && nativeReadOptionsFingerprint == that.nativeReadOptionsFingerprint
    ...
    @Override
    public int hashCode() {
        int result = Objects.hash(table.name(), filter, partitionFilter);
        result = 31 * result + Objects.hash(readType);
        result = 31 * result + nativeReadOptionsFingerprint;
        return result;
    }

    private static int nativeReadOptionsFingerprint(Map<String, String> options) {
        int result = 1;
        for (Map.Entry<String, String> entry : new TreeMap<>(options).entrySet()) {
            String key = entry.getKey();
            if (key.startsWith("native-io.")
                    || key.startsWith("__paimon.internal.native-io.")
                    || key.startsWith("fs.obs.")) {
                result = 31 * result + Objects.hash(key, entry.getValue());
            }
        }
        return result;
    }
