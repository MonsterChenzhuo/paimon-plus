...
public class DataFileRecordReader implements FileRecordReader<InternalRow> {
    ...
    private static FileRecordReader<InternalRow> createReader(
        ...
        try {
            ...
            if (hasNonCorruptFileReadException(e)) {
    ...
    static boolean ignoreCorruptException(Throwable e, boolean ignoreCorruptFiles) {
        if (hasNonCorruptFileReadException(e)) {
            return false;
        }
    ...
    private static boolean hasNonCorruptFileReadException(Throwable e) {
        Throwable current = e;
        while (current != null) {
            if (current instanceof NonCorruptFileReadException) {
                return true;
            }
