...
class SchemaValidationTest {
    ...
    @Test
    public void testRejectInternalTableOptions() {
        Map<String, String> options = new HashMap<>();
        options.put("__paimon.internal.native-io.engine", "spark");

        assertThatThrownBy(() -> validateTableSchemaExec(options))
                .hasMessageContaining("__paimon.internal.");
    }

    @Test
    public void testAllowInternalTableOptionsForDynamicCopy() {
        List<DataField> fields =
                Arrays.asList(
                        new DataField(0, "f0", DataTypes.INT()),
                        new DataField(1, "f1", DataTypes.INT()));
